...
const authenticateToken = async (req, res, next) => {
    ...
    try {
        const decoded = jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET || 'imf-africa-secret-key');
        const user = await index_1.User.findById(decoded.id).select('-password');
...
app.post('/api/auth/register', async (req, res) => {
    try {
        const { firstName, lastName, email, password, phone, country, state, city, denomination, churchName, position } = req.body;
...
app.get('/api/auth/profile', authenticateToken, async (req, res) => {
    try {
        const user = await index_1.User.findById(req.user.id).select('-password');
...
app.put('/api/auth/profile', authenticateToken, async (req, res) => {
    try {
        ...
        const user = await index_1.User.findByIdAndUpdate(req.user.id, updates, { new: true, runValidators: true }).select('-password');
